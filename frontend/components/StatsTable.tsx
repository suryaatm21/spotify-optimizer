...
export default function StatsTable({ tracks, clusters = [] }: IStatsTableProps) {
  ...
  const filteredAndSortedTracks = useMemo(() => {
    ...
    if (selectedCluster !== null) {
      filtered = tracks.filter((track) => 
        trackClusterMap.get(track.id) === selectedCluster
      );
    }
    ...
    <div className="space-y-4">
      ...
      {clusters.length > 0 && (
        <div className="flex items-center space-x-4">
          <span className="text-spotify-gray-300 font-medium">Filter by cluster:</span>
              ...
              >
                {clusters.length > 0 && (
                  <td className="p-3">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-3 h-3 rounded-full ${getClusterColor(track.id)}`}
                      />
